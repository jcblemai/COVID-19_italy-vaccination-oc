...
input_directory = f'helvetios-runs/ag-post-aggregate_{obj}'
input_prefix = f'week'
# to output the now files
output_directory = f'model_output/2021-11-17agpost_{obj}'
...
class AlternativeStrategy:
    ...
    def get_today_allocation(self, today_idx, susceptible=None, incidence=None, death=None):
        if self.compute_new_strat:
            self.alloc_arr[:, today_idx] = self.compute_today_allocation(today_idx, susceptible, incidence, death)
    ...
    def compute_today_allocation(self, today_idx, susceptible, incidence, death):
        if 'susceptible' in self.decision_variable:
            ...
            if obj == 'death':
                return self.allocate_now(death/self.divider, today_idx)
            elif obj == 'infection':
...
def create_all_alt_strategies(setup, scenario_name, scenario):
    ...
    fname = f"{input_directory}/{input_prefix}-{scenario_name}-{obj}-opt-{nnodes}_{ndays_ocp}.csv"
...
def worker_create_strategies(post_real, scenario_name, scenario, alt_strategies):
    ...
    for shortname, strat in alt_strategies.items():
        tic = time.time()
        results, state_initial, yell_infection, yell_death = COVIDVaccinationOCPagpost.accurate_integrate(setup.ndays - 1,
                                                                               setup=setup,
                                                                               parameters=p,
                                                                               controls=None,
                                                                               save_to=f'{output_directory}/{input_prefix}-{scenario_name}-{shortname}-{post_real}-{obj}',
                                                                               only_yell=False,
                                                                               alloc_strat=strat)
    ...
    for shortname, strat in alt_strategies.items():
        ...
        if isinstance(strat, AlternativeStrategy):
            ...
            results, state_initial, yell_infection, yell_death = COVIDVaccinationOCPagpost.accurate_integrate(setup.ndays - 1,
                                                                                   ...
                                                                                   alloc_strat=None)
            strat_name = strat['name']
        yell_death = results[results['comp'] == f'yell_death'].pivot(values='value', columns='place', index='date').sum().sum()
        yell_infection = results[results['comp'] == f'yell_infection'].pivot(values='value', columns='place',index='date').sum().sum()
        ...
        print(f"{scenario_name}, {post_real}, {shortname} done in {time.time()-tic:.1f} s. vacc:{vacc_tot:.0f}, yell:{yell_death:.0f}")
