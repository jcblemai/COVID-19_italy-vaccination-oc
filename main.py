...
def pick_scenario(setup, scn_id):
    ...
    if scn_spec['epicourse'] == 'C':
        ...
        course = scipy.interpolate.interp1d([0, setup.ndays / 2, setup.ndays], [1, 1.15, 1], kind='quadratic')
        course = course(np.arange(0, setup.ndays))
        scenario['beta_mult'] = np.ones((setup.nnodes, setup.ndays)) * course
    elif scn_spec['epicourse'] == 'L':
        course = scipy.interpolate.interp1d([0, setup.ndays / 2, setup.ndays], [1, .85, 1], kind='quadratic')
        course = course(np.arange(0, setup.ndays))
        scenario['beta_mult'] = np.ones((setup.nnodes, setup.ndays)) * course
    return scenario
    ...
    for scn_id in scn_ids:
        ...
        if ocp:
            ...
            ocp.update(parameters=p,
                       ...
                       mob_initial=mob,
                       scenario_name=f'{outdir}{prefix}-opt{nnodes}-nc')
